...
import { createInstitutionPOST, fetchAllOrganizations } from "@/hooks/api";
import Select from "./common/Select";
import { fetchTags } from "@/hooks/api";
import { getReadableName } from "@/utils/helpers";
import PhotoUpload from "./common/PhotoUpload";

export default function AddInstitutionForm({ closeModal, session }) {
  ...
  const onSubmit = (data) => {
    ...
    //set image to empty icon if not provided
    if (!data.image) {
      data.image = "/img/emptyInstitution.png";
    }
    createInstitutionPOST({ data, session });
    closeModal();
  };
          ...
          <div className="rounded-md shadow-sm -space-y-px p-6">
            ...
            <div className="pt-2 mb-10">
              <fieldset className="mt-2 grid grid-cols-1 border-t border-b border-gray-200 mb-10">
                {tags
                  ?.filter((t) => t.tagType === "institution")
                  .map((tag) => (
